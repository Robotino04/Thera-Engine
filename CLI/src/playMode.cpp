...
static void printBoard(Thera::Board& board, Thera::MoveGenerator& generator, std::array<RGB, 64> const& squareHighlights, Options const& options) {
    ...
    for (int y = 7; y >= 0; y--) {
        ...
        for (int x = 0; x < 8; x++) {
            ...
            // print the piece
            std::cout << pieces.at(board.at(square)) << " " << ANSI::reset();
        }
        ...
        switch (7 - y) {
            case 0:
                std::cout << (board.getColorToMove() == Thera::PieceColor::White ? "White" : "Black") << " to move.";
                std::cout << "  (eval: " + std::to_string(Thera::evaluate(board, generator)) + ")";
                break;
            case 1: std::cout << "Castling: [White] [Black]"; break;
            case 2:
                std::cout << "          ";
            ...
            case 3:
                switch (options.selectedBitboard) {
                    case BitboardSelection::AllPieces:
                        ...
                        if (options.shownPieceBitboard.type != Thera::PieceType::None) {
                            std::cout << (options.shownPieceBitboard.color == Thera::PieceColor::White ? "white " : "black ");
                        }
                        std::cout << "pieces";
                        break;
                    case BitboardSelection::Debug:        std::cout << "Showing debug bitboard"; break;
                    case BitboardSelection::PinnedPieces: std::cout << "Showing pinned pieces"; break;
                    case BitboardSelection::SinglePiece:
                        std::cout << "Showing bitboard for "
                                  << Thera::Utils::pieceToString(options.shownPieceBitboard, true);
                        break;
                    case BitboardSelection::AttackedSquares: std::cout << "Showing attacked squares"; break;
                    case BitboardSelection::None:            std::cout << "Showing no bitboard"; break;
                    case BitboardSelection::AttackedBySquares:
                        std::cout << "Showing squares attacked by " << ANSI::set4BitColor(ANSI::Blue)
                                  << Thera::Utils::squareToAlgebraicNotation(options.squareSelection)
                                  << ANSI::reset();
                        break;
                    case BitboardSelection::AttackingSquares:
                        std::cout << "Showing squares attacking " << ANSI::set4BitColor(ANSI::Blue)
                                  << Thera::Utils::squareToAlgebraicNotation(options.squareSelection)
                                  << ANSI::reset();
                        break;
                ...
                break;
            case 4:
                std::cout << "FEN: " << ANSI::set4BitColor(ANSI::Blue, ANSI::Foreground) << board.storeToFEN()
                          << ANSI::reset();
                break;
            case 5:
                std::cout << "Zobrist hash: " << ANSI::set4BitColor(ANSI::Blue, ANSI::Foreground) << std::hex
                          << std::uppercase << board.getCurrentHash() << std::dec << std::nouppercase
                          << ANSI::reset();
                break;
            case 6:
                if (Thera::evaluate(board, generator) == Thera::evalInfinity) {
                    std::cout << ANSI::set4BitColor(ANSI::Green)
                              << Thera::Utils::pieceColorToString(board.getColorToMove()) << " wins!"
                              << ANSI::reset();
                }
                else if (Thera::evaluate(board, generator) == -Thera::evalInfinity) {
                    std::cout << ANSI::set4BitColor(ANSI::Green)
                              << Thera::Utils::pieceColorToString(board.getColorToNotMove()) << " wins!"
                              << ANSI::reset();
                }
...
static void setBitboardHighlight(
    Options const& options, Thera::Board const& board, Thera::MoveGenerator& generator, std::array<RGB, 64>& highlights
) {
    ...
    switch (options.selectedBitboard) {
        ...
        case BitboardSelection::AttackedBySquares:
            bitboard = generator.getSquaresAttackedBy(options.squareSelection);
            break;
        case BitboardSelection::AttackingSquares:
            bitboard = generator.getSquaresAttacking(options.squareSelection);
            break;
        case BitboardSelection::PossibleMoveTargets: bitboard = generator.getPossibleMoveTargets(); break;

        default:                                     throw Thera::Utils::NotImplementedException();
    }
...
static void redrawGUI(
    Options const& options, Thera::Board& board, Thera::MoveGenerator& generator, std::array<RGB, 64>& highlights, std::string const& message
) {
    std::cout << ANSI::reset() << "-------------------\n" << message << ANSI::reset() << "\n";
 ...
 * @param message
 */
static void analyzePosition(
    MoveInputResult const& userInput, Thera::Board& board, Thera::MoveGenerator& generator, std::string& message, int originalDepth, bool instrumented
) {
    if (userInput.perftDepth == 0)
        return;
    if (userInput.perftDepth == originalDepth)
        message = "";
    ...
    while (true) {
        char c_buffer[1024];
        if (fgets(c_buffer, sizeof(c_buffer), out.get()) == nullptr)
            break;
        ...
        int i = 0;
        if (Thera::Utils::isInRange(buffer.at(i++), 'a', 'h') && Thera::Utils::isInRange(buffer.at(i++), '1', '8')
            && Thera::Utils::isInRange(buffer.at(i++), 'a', 'h') && Thera::Utils::isInRange(buffer.at(i++), '1', '8')) {
    ...
    if (instrumented)
        theraResult = Thera::perft_instrumented(board, generator, userInput.perftDepth, true);
    else
        theraResult = Thera::perft(board, generator, userInput.perftDepth, true);
    ...
    for (auto move : theraResult.moves) {
        if (std::find(stockfishResult.moves.begin(), stockfishResult.moves.end(), move)
            == stockfishResult.moves.end()) {
    ...
    for (auto [move, source] : differentMoves) {
        ...
        message += Thera::Utils::squareToAlgebraicNotation(move.move.startIndex)
        ...
        if (userInput.perftDepth > 0) {
            auto moveIt = std::find_if(theraResult.moves.begin(), theraResult.moves.end(), [move](auto other) {
                return move.move == other.move;
            });
    ...
    while (duplicate != theraRawMoves.end()) {
        ...
        message += Thera::Utils::squareToAlgebraicNotation(duplicate->startIndex)
                 + Thera::Utils::squareToAlgebraicNotation(duplicate->endIndex);
    ...
    message += indentation + "Stockfish searched " + std::to_string(stockfishResult.moves.size()) + " moves ("
             + std::to_string(stockfishResult.numNodesSearched) + " nodes)\n";
    message += indentation + "Thera searched " + std::to_string(theraResult.moves.size()) + " moves ("
             + std::to_string(theraResult.numNodesSearched) + " nodes)\n";
    ...
    if (differentMoves.size() || theraResult.numNodesSearched != stockfishResult.numNodesSearched)
        ...
        message += indentation + ANSI::set4BitColor(ANSI::Green) + "identical"
                 + ANSI::set4BitColor(ANSI::Blue) + ".\n";
}

void searchIterationEndCallback(Thera::SearchResult const& result) {
    std::cout << "Finished depth " << result.depthReached
              << " (" + std::to_string(result.nodesSearched) + " nodes)\n";
    if (result.isMate) {
    ...
    while (true) {
        ...
        if (options.autoplayDepth) {
            ...
            if (computerColor == board.getColorToMove() && lastOp != MoveInputResult::UndoMove) {
                ...
                auto moves = Thera::search(
                    board,
                    generator,
                    options.autoplayDepth,
                    options.autoplaySearchTime,
                ...
                if (moves.moves.size() == 0) {
                    message = ANSI::set8BitColor(warningColor8Bit) + "No moves possible! (Time: "
                            + std::to_string(duration.count()) + ")" + ANSI::reset();
                    computerColor.reset();
                    ...
                    message = "Played computer move (Depth: " + std::to_string(moves.depthReached)
        ...
        if (userInput.op == MoveInputResult::Exit)
            break;
            ...
            if (!userInput.perftInstrumented)
                message += ANSI::set8BitColor(warningColor8Bit)
                         + "Performed fast analysis. No filtering was performed!";
            lastOp = userInput.op;
            continue;
        }
            ...
            if (moves.moves.size() == 0) {
                message += ANSI::set8BitColor(warningColor8Bit)
                         + "No moves possible! (Time: " + std::to_string(duration.count()) + ")" + ANSI::reset();
            }
            else {
                auto bestMove = getRandomBestMove(moves);

                message += ANSI::set4BitColor(ANSI::Blue) + "Best move: " + bestMove.move.toString()
                         + " (Eval: ";
                if (moves.isMate) {
                ...
                message += "  Depth: " + std::to_string(moves.depthReached)
                         + "  Time: " + std::to_string(duration.count()) + ")" + ANSI::reset();
            }
            ...
            if (userInput.perftInstrumented)
                result = Thera::perft_instrumented(board, generator, userInput.perftDepth, true);
            else
                result = Thera::perft(board, generator, userInput.perftDepth, true);
            ...
            message += "Time spent: " + std::to_string(duration.count()) + "s ("
                     + std::to_string((float(result.numNodesSearched) / duration.count()) / 1000'000)
                     + "MN/s)\n";
            if (!userInput.perftInstrumented)
                message += ANSI::set8BitColor(warningColor8Bit)
                         + "Performed fast analysis. No filtering was performed!";
        ...
        for (auto move : possibleMoves) {
            message += "" + Thera::Utils::squareToAlgebraicNotation(move.startIndex)
        ...
        if (userInput.op == MoveInputResult::Exit)
            ...
            auto moveIt = std::find_if(possibleMoves.begin(), possibleMoves.end(), [&](auto other) {
                return Thera::Move::isSameBaseMove(userInput.move, other);
            });
