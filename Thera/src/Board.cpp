    ...
    while (++charIndex < fen.size()) {
        ...
        switch (tolower(c)) {
            case '/':
                ...
                x = 0;
                break;
            case '0' ... '8': x += c - '0'; break;
            case ' ':         goto parse_turn;
            default:          {
                Piece piece;
                try {
...
parse_turn:
    ...
    while (fen.at(charIndex) != ' ') {
        switch (fen.at(charIndex)) {
            ...
            default:  throw std::invalid_argument(generateFenErrorText(fen, charIndex)); break;
        }
        charIndex++;
    }
parse_en_passant:
...
std::string Board::storeToFEN() const {
    ...
    fen += ' ';
    if (currentState.canWhiteCastleLeft)
        fen += 'Q';
    if (currentState.canWhiteCastleRight)
        fen += 'K';
    if (currentState.canBlackCastleLeft)
        fen += 'q';
    if (currentState.canBlackCastleRight)
        fen += 'k';

    if (!(currentState.canWhiteCastleLeft || currentState.canWhiteCastleRight
          || currentState.canBlackCastleLeft || currentState.canBlackCastleRight)) {
        fen += '-';
    }
...
void Board::applyMoveStatic(Move const& move) {
    if (move.startIndex == move.endIndex)
        return;
    ...
    if (move.isCastling) {
        ...
        currentState.zobristHash ^=
            zobristTable.at(castlingMove.startIndex.getIndex64()).at(castlingMove.piece.getRaw());
