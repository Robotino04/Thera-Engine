...
Coordinate squareFromAlgebraicNotation(std::string const& str) {
    if (str.size() != 2)
        throw std::invalid_argument("\"" + str + "\" isn't a valid square");
    if (!Utils::isInRange(str[0], 'a', 'h'))
        throw std::invalid_argument("\"" + str + "\" isn't a valid square");
    if (!Utils::isInRange(str[1], '1', '8'))
        throw std::invalid_argument("\"" + str + "\" isn't a valid square");
...
std::string squareToAlgebraicNotation(Coordinate square) {
    if (!Utils::isInRange<uint8_t>(square.x, 0, 7) || !Utils::isInRange<uint8_t>(square.y, 0, 7))
        throw std::invalid_argument(
            std::to_string(square.x) + ";" + std::to_string(square.y) + " isn't a valid square"
        );
