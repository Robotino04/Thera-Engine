...
Move Move::fromString(std::string const& str) {
    Move move;
    if (!Utils::isInRange<int>(str.size(), 4, 5))
        throw std::invalid_argument("\"" + str + "\" isn't a valid move");
    ...
    if (str.size() == 5) {
        switch (std::tolower(str.at(4))) {
            ...
            default:  throw std::invalid_argument("'" + str.substr(4, 1) + "' isn't a valid promotion type!");
        }
    }
    return move;
}
