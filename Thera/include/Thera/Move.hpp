...
struct Move {
    ...
    constexpr Move(Coordinate start, Coordinate end, Piece piece)
        : startIndex(start), endIndex(end), piece(piece) {}
    constexpr Move() {}
    ...
    constexpr static bool isSameBaseMove(Move a, Move b) {
        return a.startIndex == b.startIndex && a.endIndex == b.endIndex && a.promotionType == b.promotionType;
    }
};
